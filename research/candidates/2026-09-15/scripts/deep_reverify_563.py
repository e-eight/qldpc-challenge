...
doc = json.load(open("/tmp/563-18-20.json"))
n = doc["n"]
...
for trials, seeds in ((300_000, (77001, 77002, 77003, 77004)), (2_000_000, (77101, 77102, 77103))):
    t = time.time(); best = n + 1; sides = {}
    for s in seeds:
        w, side, sup = gf2_fast.distance_rand_witness(HX, HZ, int(trials), int(s), 8, 8)
        if w <= n:
            best = min(best, w); sides.setdefault(side, []).append(w)
