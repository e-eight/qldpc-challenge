...
struct Worker {
    ...
    Worker(const Prepared& p, uint64_t seed, int pair_depth, bool use_mask, int block = 1, uint64_t restart = 0, int proposals_per_sample = 8)
        : prepared(p), work(p.kernel.rows(), p.kernel.columns()),
          witnesses(p.kernel.columns(), p.kernel.columns()), table(1 << block, p.kernel.columns()), rows(p.kernel.rows()),
    ...
    void trial() {
        const int count = work.rows(), words = work.words();
        if (!restart_interval || completed % restart_interval == 0 || nonpivots.empty()) fresh_basis();
        else exchange_basis();
        for (int i = 0; i < count; ++i) {
            int weight = 0;
            for (int w = 0; w < words; ++w) weight += __builtin_popcountll(rows[i][w]);
            weights[i] = weight;
            if (weight > 0 && weight < best && nontrivial(rows[i])) retain(weight, rows[i]);
        }
        if (pairs > 1) {
            ...
            for (int i = 0; i < pairs; ++i) for (int j = i + 1; j < pairs; ++j) {
                ...
                if (weight > 0 && weight < best && nontrivial(a, b)) retain(weight, a, b);
            }
        }
        ++completed;
    }
    ...
    std::size_t bytes() const {
        return work.bytes() + witnesses.bytes() + table.bytes() + rows.size() * sizeof(uint64_t*) +
