...
from dispatch_search import CONFIG, Search, detector
from inspect_dispatch import create_corpus
from study_candidates import archive_sources as base_archive
from study_external import run_side
from study_strategies import summarize

METHODS = ("incremental", "guided", "circulant", "dispatch")


def archive_sources(output):
    sources, binaries = base_archive(output)
    extra = [
        HERE / name
        for name in (
            "study_dispatch.py",
            ...
            "audit_dispatch.py",
            "audit_candidates.py",
            "strategy_prototypes/DISPATCH_PLAN.md",
        )
    ]
    extra += [ROOT / "native/structure_dispatch" / name for name in ("detect.cpp", "setup.py", "README.md")]
    with tarfile.open(output / "dispatch-sources.tar.gz", "w:gz") as archive:
        for path in extra:
            relative = str(path.relative_to(ROOT))
            sources[relative] = sha256(path)
            archive.add(path, arcname=relative)
    binary = Path(detector.__file__)
...
def main(args):
    ...
    env = dict(
        ...
        fallback="Rejected detector or unhelpful pilot yields to guided; accepted route reserves 20 percent for guided",
    ...
    with ProcessPoolExecutor(
        ...
        for case in cases:
            ...
            for seed in range(args.seed_start, args.seed_start + args.seeds):
                ...
                random.Random(f"dispatch-v1:{case['id']}:{seed}").shuffle(methods)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--corpus", type=Path, default=HERE / "results/dispatch-study/corpus")
