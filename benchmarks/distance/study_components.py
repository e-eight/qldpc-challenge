"""Frozen construction-diverse component comparison."""
...
from strategy_prototypes.block_collision.adapter import native
from strategy_prototypes.component_search.adapter import CONFIG, METHODS, Search
from study_block_collision import archive_sources as prior_archive
...
def archive_sources(output):
    ...
    extra = [HERE / "study_components.py", HERE / "audit_components.py"]
    extra += list((HERE / "strategy_prototypes/component_search").glob("*.py"))
    extra += [HERE / "strategy_prototypes/component_search" / name for name in ("PLAN.md",)]
    with tarfile.open(output / "component-sources.tar.gz", "w:gz") as archive:
        for path in extra:
...
def main(args):
    ...
    env = dict(
        ...
        parameters=CONFIG,
        threadpools=pools,
        reference_policy="Only matrices, method and documented blocks enter Search",
    ...
    with ProcessPoolExecutor(
        ...
        for case in cases:
            ...
            for seed in range(args.seed_start, args.seed_start + args.seeds):
                ...
                random.Random(f"component-v1:{case['id']}:{seed}").shuffle(methods)
                for method in methods:
                    ...
                    for side, own, opposite in (("X", hx, hz), ("Z", hz, hx)):
                        ...
                        sides[side] = [
                            run_side(
                                Search(own, opposite, method, case["blocks"] if method != "auto-components" else ()),
                                args.seconds / 2,
                                side_seed,
                                folder / f"{side}.jsonl",
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--methods", nargs="+", choices=METHODS + ("exact-components",), default=list(METHODS))
