"""Aggregate completed allocation grids without rerunning search."""
...
def main():
    grids = {}
    for name, count, budget, seeds in (("screen-2s", 117, 2, 3), ("original-10s", 63, 10, 3)):
        directory = BASE / name
        env = json.loads((directory / "environment.json").read_text())
        audit = json.loads((directory / "audit-allocation.json").read_text())
    ...
    inputs = [
        BASE / name / file
        for name in grids
        for file in ("environment.json", "audit-allocation.json", "summary.json", "results.json")
    ]
    ...
    paired, unsupported, race_counters, regressions = {}, [], [], []
    for name, grid in grids.items():
        ...
        for record in grid["records"]:
            ...
            race_counters.extend(cs)
            for control in ("guided", "dispatch"):
                other = next(
                    r
                    for r in grid["records"]
                    if r["case"] == record["case"] and r["seed"] == record["seed"] and r["method"] == control
                )
                a = min(v["best_in_budget"] for v in record["sides"].values())
                b = min(v["best_in_budget"] for v in other["sides"].values())
                if a > b:
                    regressions.append(
                        dict(
                            grid=name,
            ...
            if all(c["route_decision"] == "no_partition" for c in cs):
                ...
                unsupported.append(
                    dict(
                        ...
                        case=record["case"],
                        seed=record["seed"],
                        race=min(v["best_in_budget"] for v in record["sides"].values()),
    ...
    decisions = dict(
        ...
        max_detection_seconds=max(c["detection_seconds"] for c in race_counters),
        max_detection_overrun_seconds=max(
            max(0, c["detection_seconds"] - c["detection_allowance_seconds"]) for c in race_counters
        ),
        max_detector_workspace_bytes=max(d["workspace_bytes"] for c in race_counters for d in c["detection"]),
        allocation=[dict(grid=name, **row) for name, grid in grids.items() for row in grid["summary"]["allocation"]],
