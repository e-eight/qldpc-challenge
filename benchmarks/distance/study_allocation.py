...
# isort: split
from allocation_search import CONFIG, Search
from common import HERE, ROOT, atomic_json, code_target, matrix_hash, sha256
from inspect_dispatch import create_corpus
from study_dispatch import archive_sources as dispatch_archive
from study_external import run_side
from study_strategies import summarize

METHODS = ("guided", "dispatch", "race")


def archive_sources(output):
    sources, binaries = dispatch_archive(output)
    extra = [
        HERE / name
        for name in (
            "allocation_search.py",
            "study_allocation.py",
            "audit_allocation.py",
            "test_allocation.py",
            "strategy_prototypes/ALLOCATION_PLAN.md",
            "strategy_prototypes/structure_route_v2/test_adapter.py",
            "strategy_prototypes/structure_route_v2/adapter.py",
        )
    ]
    with tarfile.open(output / "allocation-sources.tar.gz", "w:gz") as archive:
        for path in extra:
...
def main(args):
    ...
    env = dict(
        ...
        fallback="Rejected detector yields to guided; accepted route competes with resumable guided, "
        "80/20 allocation and 10 percent advantage threshold",
    ...
    with ProcessPoolExecutor(
        ...
        for case in cases:
            ...
            for seed in range(args.seed_start, args.seed_start + args.seeds):
                ...
                random.Random(f"allocation-v1:{case['id']}:{seed}").shuffle(methods)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--corpus", type=Path, default=HERE / "results/allocation-study/corpus")
